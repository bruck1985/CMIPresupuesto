...
namespace UI.Web
{
    public partial class ConciliacionCXP : System.Web.UI.Page
    {
        bool bounded;
...
//                ((Lform.FindItemOrGroupByName("CB_Tipo_Reporte") as LayoutItem).GetNestedControl() as ASPxComboBox).Value = "A";
        ...
        protected void Cmbcentrocosto_Callback(object source, CallbackEventArgsBase e)
        {
            ...
            if (string.IsNullOrEmpty(e.Parameter)) return;
              SqlDataCategoriaProv.SelectParameters["PCia1"].DefaultValue = e.Parameter.ToString();
            Session["CIAS_OC_PRES"] = e.Parameter.ToString();
            SqlDataCategoriaProv.DataBind();
            (((Lform.FindItemOrGroupByName("categoria") as LayoutItem).GetNestedControl() as ASPxDropDownEdit).FindControl("listBoxcc") as ASPxListBox).DataBind();
  ...
  //            CmbCity.DataBind();
        ...
        protected void ASPxFormLayout1_E3_Click(object sender, EventArgs e)
        {
            ...
            LayoutItem itemCentroCosto = Lform.FindItemOrGroupByName("categoria") as LayoutItem;
            ASPxDropDownEdit CBCategoria = itemCentroCosto.GetNestedControl() as ASPxDropDownEdit;
            ...
            if (CBFechaInicial != null)
            {
                if (CBCiaOri.Value != null)
                {
                    ...
                    Session["lista_categoria"] = CBCategoria.Value != null ? CBCategoria.Value.ToString() : "XXX-yyyy ZZZZZZZZZ;";
                }
                else
                {
...
//                Session["ci_stipo"] = CBtiporep.Value != null ? CBtiporep.Value.ToString() : string.Empty;
        ...
        protected void ASPxFormLayout1_E3_ClickExc(object sender, EventArgs e)
        {
            ...
            WriteToResponse("ReporteConciliacion CXP.xlsx", true, "vnd.openxmlformats-officedocument.spreadsheetml.sheet", stream);
